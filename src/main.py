...
def main_elpais():
    try:
        raw_data = get_elpais_data(num_search_pages=2)
        transformed_data = transform_elpais_to_df(raw_data)
        now = datetime.now().date()
        transformed_data['_created'] = now
        transformed_data['source'] = 'elpais'
        file_name = '/tmp/data_elpais.parquet'
        transformed_data.to_parquet(file_name, index=False)
        upload_to_s3(file_name, bucket_name='scraper-meli', object_name=f'carros/data_{now}_elpais.parquet')
