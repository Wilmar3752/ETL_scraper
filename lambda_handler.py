...
def handler(event, context):
    ...
    if source in ("autonal", "all"):
        try:
            main_autonal()
            results["autonal"] = "success"
            logger.info("ETL completed for autonal")
        except Exception as e:
            results["autonal"] = f"error: {str(e)}"
            logger.error(f"ETL failed for autonal: {str(e)}")

    if source in ("elpais", "all"):
        try:
            main_elpais()
            results["elpais"] = "success"
            logger.info("ETL completed for elpais")
        except Exception as e:
            results["elpais"] = f"error: {str(e)}"
            logger.error(f"ETL failed for elpais: {str(e)}")
    ...
    if source == "motor":
        try:
            main_motor()
            results["motor"] = "success"
            logger.info("ETL completed for motor")
        except Exception as e:
            results["motor"] = f"error: {str(e)}"
            logger.error(f"ETL failed for motor: {str(e)}")
